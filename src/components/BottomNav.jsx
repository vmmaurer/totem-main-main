...
/* ----------------------------------------------------------------
   Liquid glass bottom nav — direct port of the reference "switcher"
   technique (CSS custom properties + color-mix layered box-shadows
   + an SVG feDisplacementMap filter inside backdrop-filter), tuned
   for a dark background: the bar itself stays almost colorless and
   gets its "dark glass" look from blurring/saturating whatever is
   behind it, while the active tab sits on a slightly lighter,
   glossy pill — exactly like the reference screenshot.
------------------------------------------------------------------ */
...
const GlassFilterDefs = () => (
  ...
  >
    <filter id="bottomNavLiquidGlass" primitiveUnits="objectBoundingBox">
...
const Nav = styled.nav`
  ...
  backdrop-filter: blur(10px) url(#bottomNavLiquidGlass) saturate(var(--saturation));
